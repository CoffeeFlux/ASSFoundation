return (ASS, ASSFInst, yutilsMissingMsg, createASSClass, Functional, LineCollection, Line, logger, SubInspector, Yutils) ->
    ...
    Contour.getExtremePoints = (allowCompatible) =>
        ...
        for i = 1, #@commands
            ...
            for pt in *pts
                top = pt if not top or top.y > pts[i].y
                left = pt if not left or left.x > pts[i].x
                bottom = pt if not bottom or bottom.y < pts[i].y
                right = pt if not right or right.x < pts[i].x
